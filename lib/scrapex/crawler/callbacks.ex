defmodule Scrapex.Crawler.Callbacks do
  @callbacks [:before_start, :after_start, :before_pause,
              ...
              :before_parse_enqueue, :before_parse_start, :after_parse_complete,
              :before_data_enqueue, :before_data_process_start, :after_data_processed]

  defmacro __using__(_) do
    quote do
      import unquote(__MODULE__), except: [trigger: 2]
  ...
  defmacro __before_compile__(_) do
    quote do
      def trigger_callback(callback_name) do
        Scrapex.Crawler.Callbacks.trigger(__MODULE__, callback_name)
      end
  ...
  def trigger(module, callback_name) do
    callbacks = Keyword.get(module.defined_callbacks(), callback_name, [])

    Enum.each(callbacks, fn (fun) ->
      cond do
        is_function(fun) -> fun.()
        is_atom(fun)     -> apply(module, fun, [])
